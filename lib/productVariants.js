...
function alignImagesByPrimaryAttribute(variants, attrs) {
  ...
  /**
   * Một giá trị phân loại 1 (vd. một màu) → chỉ một bộ ảnh, dù có nhiều dòng phân loại 2 (size…).
   * Không gộp ảnh từ nhiều dòng (FE đôi khi gửi mỗi dòng 1 URL → lặp thành 3 ảnh).
   * Dòng sau ghi đè dòng trước cùng phân loại 1 — sửa ảnh ở bất kỳ dòng nào (thường là dòng cuối) vẫn lưu đúng.
   */
  ...
  for (const variant of variants) {
    ...
    const rowGallery = gallery.length
      ? [...new Set(gallery)]
      : fallbackImage
        ? [fallbackImage]
        : []
    if (!rowGallery.length) continue
    galleryByPrimaryValue.set(primaryValue, rowGallery)
  }
